...
from travel_agent.utils.travel_date import (
    build_travel_date_question,
    infer_travel_date,
    is_flexible_travel_date,
    parse_travel_date,
)
...
ORIGIN_CITIES = ["上海", "北京", "广州", "深圳", "杭州", "成都", "重庆", "武汉", "南京", "厦门", "西安", "青岛", "天津"]
...
# 辅助函数
# ============================================================
def _parse_travel_date(text: str) -> str | None:
    return parse_travel_date(text)
...
def _parse_answer(profile: UserProfile, text: str):
    ...
    if text in BUDGET_OPTION_MAP:
        profile.budget_total = BUDGET_OPTION_MAP[text]

    if text in COMPANION_OPTION_MAP:
        from travel_agent.models.user_profile import CompanionInfo
        count, relation = COMPANION_OPTION_MAP[text]
        if profile.travel_type == "蜜月旅行" and count == 2:
            relation = "配偶"
        profile.companions = [CompanionInfo(relation=relation, count=count)]

    if text in ORIGIN_CITIES:
        profile.origin = text
    else:
        origin_match = re.search(r"从\s*([^\s，,。到至去]+?)\s*出发", text)
        if origin_match:
            profile.origin = origin_match.group(1).strip()
        elif text.endswith("出发") and len(text) <= 8:
            profile.origin = text.replace("出发", "").strip("从")

    if is_flexible_travel_date(text):
        profile.notes = (profile.notes or "") + " | 时间灵活，希望黄历择日"
        return

    travel_date = _parse_travel_date(text)
    if travel_date:
    ...
    if days_range_pattern:
        ...
        elif "7天以上" in text or "7天+" in text:
            profile.duration_days_range = "7天以上"
            profile.duration_days = 8
...
def _get_llm_decision(state: AgentState) -> dict:
    ...
    try:
        ...
        if result and isinstance(result, dict) and result.get("action"):
            ...
            return _ensure_city_origin_options(result, profile)
    ...
    # 追问缺失项
    question, options = _get_missing_info_question(missing)
    return {"action": "ask", "question": question, "options": options}


def _ensure_city_origin_options(decision: dict, profile: UserProfile) -> dict:
    """追问出发地时，强制使用具体城市选项，避免模型给出京津冀等区域词"""
    if decision.get("action") != "ask" or _find_missing_info(profile) != "origin":
        return decision
    _, city_options = _MISSING_INFO_QUESTIONS["origin"]
    decision["options"] = list(city_options)
    return decision


def _get_missing_info_question(missing: str | None) -> tuple[str, list]:
    if missing == "travel_date":
        return build_travel_date_question()
    return _MISSING_INFO_QUESTIONS.get(missing, ("请问您还有什么特别的需求吗？", []))
...
_MISSING_INFO_QUESTIONS = {
    ...
    "origin": ("您从哪个城市出发呢？", [
        {"value": "北京", "emoji": "✈️", "label": "北京", "desc": "首都/大兴机场"},
        {"value": "上海", "emoji": "✈️", "label": "上海", "desc": "浦东/虹桥机场"},
        {"value": "广州", "emoji": "✈️", "label": "广州", "desc": "白云机场"},
        {"value": "深圳", "emoji": "✈️", "label": "深圳", "desc": "宝安机场"},
        {"value": "杭州", "emoji": "✈️", "label": "杭州", "desc": "萧山机场"},
        {"value": "成都", "emoji": "✈️", "label": "成都", "desc": "双流/天府机场"},
        {"value": "其他", "emoji": "✏️", "label": "其他", "desc": "输入具体城市名"},
    ]),
    "companions": ("一共几个人一起出行呢？", [
        {"value": "1人（独自）", "emoji": "🎒", "label": "1人", "desc": "独自探索"},
        {"value": "2人（情侣/朋友）", "emoji": "💑", "label": "2人", "desc": "情侣或好友同行"},
        {"value": "3-5人（家庭/小团体）", "emoji": "👨‍👩‍👧", "label": "3-5人", "desc": "家庭或小团体"},
        {"value": "6人以上（团建）", "emoji": "👥", "label": "6人+", "desc": "多人团建"},
        {"value": "其他", "emoji": "✏️", "label": "其他", "desc": "自由描述"},
    ]),
}


def _companions_known(profile: UserProfile) -> bool:
    if profile.companions:
        return True
    from travel_agent.models.user_profile import CompanionInfo
    if profile.travel_type == "独自旅行":
        profile.companions = [CompanionInfo(relation="独自", count=1)]
        return True
    if profile.travel_type == "蜜月旅行":
        profile.companions = [CompanionInfo(relation="配偶", count=2)]
        return True
    return False


def _travel_date_known(profile: UserProfile) -> bool:
    if profile.travel_date:
        return True
    notes = profile.notes or ""
    return any(hint in notes for hint in ("时间灵活", "黄历择日", "帮我择日", "灵活择日"))


def _origin_known(profile: UserProfile) -> bool:
    return bool((profile.origin or "").strip())
...
def _find_missing_info(profile: UserProfile) -> str | None:
    ...
    if not profile.budget_total or profile.budget_total <= 0:
        return "budget"
    if not _origin_known(profile):
        return "origin"
    if not _companions_known(profile):
        return "companions"
    if not _travel_date_known(profile):
        return "travel_date"
    return None
...
def _do_plan(state: AgentState, profile: UserProfile, retriever, coordinator: MultiAgentCoordinator | None = None):
    ...
    conversation_summary = _build_conversation_summary(state)
    inferred_date = infer_travel_date(profile, conversation_summary)
    if inferred_date:
        profile.travel_date = inferred_date
...
def _generate_plan_with_llm(
    ...
    for item in tool_outputs:
        ...
        preview_lines = 50 if tool_name == "calendar_lookup" else (15 if tool_name in {"flight_search", "hotel_search"} else 10)
        preview = "\n".join(summary.splitlines()[:preview_lines]).strip()
...
4. 如果「实时工具结果」中有 flight_search / 机票 数据：必须单独输出「## 交通参考（机票）」小节，列出具体航班号、起降机场、起降时间、是否中转；价格标注为「1成人参考价」，不要按同行人数折算总价
5. 如果 flight_search 在「已跳过/失败的实时工具」中，或实时工具结果里没有机票数据：不要编造航班号、具体价格或班次；交通预算写「待查询」或「预留 XX 元」，行程中写「搭乘航班抵达（具体班次待查询）」
6. 每天至少安排 4 个时段：上午 / 午餐 / 下午 / 晚餐
7. 每天行程要有区别，避免前后天重复
8. 预算要合理分配，并给出总预算和小结；预算摘要必须完整输出六项：总预算、交通、餐饮、住宿、购物、其他，其中“其他”不能省略，且每一项都要有明确金额
9. 输出必须是 Markdown，不要输出 JSON，不要输出代码块，不要加解释性前缀
10. 直接输出最终规划正文，要求可直接展示给用户
11. 若实时工具结果与攻略笔记冲突，优先采用实时工具结果，并在小贴士里简短提示"以实时查询为准"
12. 仅当「实时工具结果」中包含 calendar_lookup / 黄历 / 农历 数据时，才输出「## 出行择日建议」小节
13. 有黄历数据时：推荐出发日必须写具体 YYYY-MM-DD，并引用该日的宜忌、建除、值神、冲煞作为依据；至少给出 2 个候选日
14. 若没有黄历数据：不要输出「## 出行择日建议」小节，不要写“系统暂未获取实时黄历数据”，不要用“周二/周三错峰”代替黄历
    ...
    def select_option():
        try:
            ...
            # 3. SPECIAL_REQUIREMENTS 阶段（用户已选了要补充哪方面，追问具体内容）
                ...
                if option_value == "补充人数/日期":
                    profile.notes = (profile.notes or "") + f" | [{label}]"
                    if not _companions_known(profile):
                        question, options = _MISSING_INFO_QUESTIONS["companions"]
                    else:
                        question, options = build_travel_date_question()
                    _send_question(state, question, options)
                    state["current_stage"] = Stages.TRAVEL_TYPE
                    sessions[session_id] = state
                    ...
                    return jsonify({
                        "session_id": session_id,
                        "messages": [msg.model_dump() for msg in state["messages"]],
                        "next_question": latest.content,
                        "next_options": latest.options,
                        "current_stage": state["current_stage"],
                        "trip_plan": state["trip_plan"],
                    })
