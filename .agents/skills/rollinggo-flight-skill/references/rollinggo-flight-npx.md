...
### 临时执行（npx，无需安装）
```bash
npx --yes rollinggo-flight@latest --help
npx --yes rollinggo-flight@latest search-airports --keyword "Hangzhou"
```

### 全局安装（适合重复使用）
```bash
npm install -g rollinggo-flight@latest
...
## 版本新鲜度
默认策略：每次执行都确保使用 npm 最新版本。

```bash
npx --yes rollinggo-flight@latest <subcommand> ...
```
...
## 命令说明
以下示例为了易读，统一使用已安装的 `rollinggo-flight` 命令。若要强制走最新版，对应前缀为 `npx --yes rollinggo-flight@latest`。
